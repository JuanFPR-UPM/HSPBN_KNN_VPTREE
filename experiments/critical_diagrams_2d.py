...
from critdd import stats, tikz, tikz_2d
from critdd.stats import FDistributedFriedmanResult, ChiSquareFriedmanResult
...
class Diagram(AbstractDiagram):
    ...
    def __init__(self, X, *, treatment_names=None, maximize_outcome=False):
        if treatment_names is None:
            treatment_names = [ f"treatment {i}" for i in range(X.shape[1] - 1) ]
        elif len(treatment_names) != X.shape[1] - 1:
            raise ValueError("len(treatment_names) != X.shape[1]")
        self.treatment_names = np.array(treatment_names)
        rankings, statistic, p_value, critical_value, hypothesis = friedman(X, alpha = 0.05, minimize= not maximize_outcome, verbose=False)

        self.r = FDistributedFriedmanResult(p_value, statistic, ChiSquareFriedmanResult(None, None, None, np.array(list(rankings.values())), None, maximize_outcome), None, None)

        num_cases = X.shape[0]
        results, _ = hommel(ranks=rankings, num_cases=num_cases, alpha=0.05, control = None, type_rank = "Friedman", verbose=False)

        self.P = create_symmetric_matrix(results).iloc[1:, 1:].to_numpy()
    ...
    def get_groups(self, alpha=.05, return_names=False, return_singletons=True):
        ...
        groups = list(nx.find_cliques(G)) # convert generator to list of cliques
...
class Diagrams(AbstractDiagram):
    ...
    def __init__(
        ...
        n_diagrams = len(Xs)
        n_treatments = Xs[0].shape[1] - 1
        if not np.all([ X.shape[1]-1 == n_treatments for X in Xs ]):
    ...
    def to_str(self, alpha=.05, **kwargs):
        return tikz_2d.to_str(
            np.stack([ d.average_ranks for d in self.diagrams ]),
            [ d.get_groups(alpha, return_singletons=False) for d in self.diagrams ],
            self.treatment_names,
            self.diagram_names,
            **kwargs
        )