...
double mi_general(VPTree& ztree,
    ...
    /////////////////////// DISABLE QUERY ONLY ON Z (COMMENT LINE ABOVE)///////////////////////

    auto xz_is_discrete_column = std::vector<bool>();
    xz_is_discrete_column.reserve(is_discrete_column.size() - 1);
    xz_is_discrete_column.push_back(is_discrete_column[0]);
    xz_is_discrete_column.insert(
        xz_is_discrete_column.end(),
        is_discrete_column.begin() + 2,
        is_discrete_column.end()
    );

    auto yz_is_discrete_column = std::vector<bool>();
    yz_is_discrete_column.reserve(is_discrete_column.size() - 1);
    yz_is_discrete_column.push_back(is_discrete_column[1]);
    yz_is_discrete_column.insert(
        yz_is_discrete_column.end(),
        is_discrete_column.begin() + 2,
        is_discrete_column.end()
    );
    auto z_is_discrete_column = std::vector<bool>(is_discrete_column.begin() + 2, is_discrete_column.end());

    std::vector<int> z_column_indices(df->num_columns() - 2);
    std::iota(z_column_indices.begin(), z_column_indices.end(), 2);

    std::vector<int> xz_column_indices;
    xz_column_indices.reserve(1 + z_column_indices.size());
    xz_column_indices.push_back(0);
    xz_column_indices.insert(
        xz_column_indices.end(),
        z_column_indices.begin(),
        z_column_indices.end()
    );

    std::vector<int> yz_column_indices;
    yz_column_indices.reserve(1 + z_column_indices.size());
    yz_column_indices.push_back(1);
    yz_column_indices.insert(
        yz_column_indices.end(),
        z_column_indices.begin(),
        z_column_indices.end()
    );

    auto xz_df = df.loc(xz_column_indices);
    auto yz_df = df.loc(yz_column_indices);
    auto z_df = df.loc(z_column_indices);

    VPTree xztree(xz_df, datatype, xz_is_discrete_column, tree_leafsize, seed);
    VPTree yztree(yz_df, datatype, yz_is_discrete_column, tree_leafsize, seed);

    auto n_xz = xztree.count_ball_unconditional(xz_df, eps, xz_is_discrete_column);
    auto n_yz = yztree.count_ball_unconditional(yz_df, eps, yz_is_discrete_column);
    auto n_z = ztree.count_ball_unconditional(z_df, eps, z_is_discrete_column);
...
/////////////////////// DISABLE TREE RECYCLING ///////////////////////
                auto z_is_discrete_column = std::vector<bool>(is_discrete_column.begin() + 2, is_discrete_column.end());
                std::vector<int> column_indices(shuffled_df->num_columns() - 2);
                std::iota(column_indices.begin(), column_indices.end(), 2);
                auto z_df = shuffled_df.loc(column_indices);
                VPTree ztree(z_df, m_datatype, z_is_discrete_column, m_tree_leafsize, m_seed);
