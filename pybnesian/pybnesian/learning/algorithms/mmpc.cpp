...
using learning::independences::hybrid::MixedKMutualInformation;
...
void recompute_assoc(const IndependenceTest& test,
    ...
    assoc.reset_maxmin();
    std::vector<int> tbc_vec(to_be_checked.begin(), to_be_checked.end());

#pragma omp parallel
    {
#pragma omp for schedule(dynamic) nowait
        for (int i = 0; i < tbc_vec.size(); ++i) {
            double pvalue = test.pvalue(variable_name, g.name(tbc_vec[i]), cpc_vec);
#pragma omp critical

            {
                assoc.initialize_assoc(tbc_vec[i], pvalue);
            }
            progress.tick();
        }
    }
...
#pragma omp for schedule(dynamic)
            for (int i = 0; i < tbc_vec.size(); ++i) {
                ...
                double pvalue_1 = test.pvalue(variable_name, v_name, last_added_name);

                double pvalue_2 = test.pvalue(variable_name, v_name, cond);
#pragma omp critical
                {
                    assoc.update_assoc(v, pvalue_1);
                    assoc.update_assoc(v, pvalue_2);
                }

                progress.tick();
            }
        } else {
            progress.set_text("MMPC Forward: sepset up to order " + std::to_string(cpc.size()) + " for " +
                              variable_name);
            progress.set_max_progress(to_be_checked.size());
            progress.set_progress(0);
            ...
            if (cpc.size() > 3) {
                ...
                all_combs = std::vector<std::vector<std::string>>(comb.begin(), comb.end());
            }
...
#pragma omp for schedule(dynamic)
                    for (int i = 0; i < all_combs.size(); ++i) {
                        auto comb = all_combs[i];
                        pvalue = test.pvalue(variable_name, v_name, comb);
#pragma omp critical
                        {
                            assoc.update_assoc(v, pvalue);
                        }
...
void mmpc_backward_phase(const IndependenceTest& test,
    ...
    if (cpc.size() > 1) {
        ...
        for (auto it = cpc.begin(), end = cpc.end(); it != end;) {
            ...
            for (int i = 0; i < subset_variables.size(); ++i) {
                auto subset_var = subset_variables[i];

                auto pvalue = test.pvalue(variable_name, it_name, subset_var);
                if (pvalue > alpha) {
                    it = cpc.erase(it);
                    progress.set_max_progress(cpc.size());
                    found_sepset = true;
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < num_total_nodes; ++i) {
        if (cpcs[i].size() == 1) {
            ...
            std::unordered_set<int> remove_i;
            for (int p : to_be_checked[i]) {
                bool repeated_test =
...
#pragma omp critical

                    {
                        ...
                        if (assoc.min_assoc(p, i) > alpha) remove_i.insert(p);
...
#pragma omp critical
                ...
                {
                    for (int r : remove_i) to_be_checked[i].erase(r);
                }
            }
        }
