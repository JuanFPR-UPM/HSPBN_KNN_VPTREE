...
public:
    ...
    GraphClass<DirectedAcyclic> to_approximate_dag(const ArcStringVector& varc_blacklist) const;
...
template <typename Derived, template <typename> typename BaseClass>
typename PartiallyDirectedImpl<Derived, BaseClass>::template GraphClass<DirectedAcyclic>
PartiallyDirectedImpl<Derived, BaseClass>::to_approximate_dag(const ArcStringVector& arc_blacklist) const {
    GraphClass<DirectedAcyclic> directed;
    ArcSet arc_blackset;
    ...
    for (const auto& arc : arc_blacklist) {
        ...
        auto s = this->index(arc.first);
        auto t = this->index(arc.second);
        
        if (this->has_edge_unsafe(s, t)) {
            this->direct(t, s);
        }
