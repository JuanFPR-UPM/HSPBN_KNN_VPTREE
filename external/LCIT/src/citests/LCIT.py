...
from ..utils.utils import strip_outliers
...
class UniformFlow(nn.Module):
    def __init__(self, dz, n_components, hidden_sizes, activator=nn.LeakyReLU, activator_params=None, batch_norm=True):
        super().__init__()
        activator_params = activator_params or {}
        self.z_to_mu = MLP(dz, n_components, hidden_sizes=hidden_sizes, activator=activator, activator_params=activator_params, norm=batch_norm)
        self.z_to_logstd = MLP(dz, n_components, hidden_sizes=hidden_sizes, activator=activator, activator_params=activator_params, norm=batch_norm)
        self.z_to_w = nn.Sequential(
            *MLP(dz, n_components, hidden_sizes=hidden_sizes, activator=activator, activator_params=activator_params, norm=batch_norm),
            nn.Softmax(dim=1)
        )
...
class CCNF(LightningModule):
    def __init__(self, dz, X_cat, Y_cat, n_components, hidden_sizes, lr, weight_decay, verbose, batch_norm):
        super().__init__()
        self.save_hyperparameters()
        self.validation_outputs = []

        self.x_cnf = UniformFlow(dz=dz, n_components=n_components, hidden_sizes=hidden_sizes, activator_params=dict(inplace=True), batch_norm=batch_norm)
        self.y_cnf = UniformFlow(dz=dz, n_components=n_components, hidden_sizes=hidden_sizes, activator_params=dict(inplace=True), batch_norm=batch_norm)

        if X_cat:
            self.x_dequants = DQFlow(dz=dz + 1, n_components=n_components, hidden_sizes=hidden_sizes, activator_params=dict(inplace=True), batch_norm=batch_norm)
        if Y_cat:
            self.y_dequants = DQFlow(dz=dz + 1, n_components=n_components, hidden_sizes=hidden_sizes, activator_params=dict(inplace=True), batch_norm=batch_norm)
...
def infer_types(X, Y, Z, rng):
    # Identify categorical flags
    X_cat = pd.api.types.is_categorical_dtype(X)
    Y_cat = pd.api.types.is_categorical_dtype(Y)

    # Construct DataFrame
    df = pd.DataFrame(np.column_stack((X, Y, Z)))

    # Manually detect categorical columns
    cat_cols = [i for i in df.columns if pd.api.types.is_categorical_dtype(df[i]) or pd.api.types.is_object_dtype(df[i])]

    N, d = df.shape
    assert N == X.shape[0]

    # Label-encode X and Y if categorical
    for col in [0, 1]:
        if col in cat_cols:
            df[col] = LabelEncoder().fit_transform(df[col])
            cat_cols.remove(col)  # Already encoded
    ...
    # Convert outputs to numpy arrays of appropriate types
    X = df[0].astype(np.float32).values
    Y = df[1].astype(np.float32).values
    Z = df.drop(columns=[0, 1]).values.astype(np.float32)

    assert N == len(X) == len(Y) == Z.shape[0]


    return (X, X_cat), (Y, Y_cat), Z

def LCIT(X, Y, Z, n_bootstraps=0, normalize=True, n_components=32, hidden_sizes=4, lr=5e-3, weight_decay=5e-5, max_epochs=100, random_state=0, gpus=0, return_latents=False, verbose=False, batch_norm=True, **kwargs):
    ...
    if normalize:
        ...
        if not Y_cat:
            ...
            Y = strip_outliers(Y)
        Z_norm = np.empty_like(Z, dtype=np.float32)
        for i in range(dz):
            col = Z[:, i]
            std = np.std(col)
            if std > 0:
                Z_norm[:, i] = strip_outliers((col - np.mean(col)) / std)
            else:
                Z_norm[:, i] = col  # leave constant columns as-is

        Z = Z_norm
    X, Y, Z = map(lambda x: torch.tensor(x, dtype=torch.float32).view(N, -1), (X, Y, Z))

    model = CCNF(dz=dz, X_cat=X_cat, Y_cat=Y_cat, n_components=n_components, hidden_sizes=hidden_sizes, lr=lr, weight_decay=weight_decay, verbose=verbose, batch_norm=batch_norm)
